...
import com.example.MusicSenso.databinding.ActivityMainBinding
import java.io.File
...
//        noMusicText = findViewById(R.id.no_songs)
        ...
        while (cursor!!.moveToNext()) {
            val column2Value = cursor.getString(2)
            if (column2Value != null) { // Check if the column value is not null
                val songData = AudioModel(cursor.getString(1), cursor.getString(0), column2Value)
                if (File(songData.path).exists()) { // Check for the existence of songs
